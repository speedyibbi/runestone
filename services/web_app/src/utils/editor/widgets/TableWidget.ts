...
export class TableWidget extends WidgetType {
  ...
  toDOM(): HTMLElement {
    ...
    // Handle double-click to edit markdown directly (only in edit mode)
    ...
    this.data.headers.forEach((header, i) => {
      ...
      th.contentEditable = this.isPreviewMode ? 'false' : 'true'
      th.spellcheck = false
      if (this.data.alignments[i]) {
        th.style.textAlign = this.data.alignments[i]
      }

      // Add editing functionality only in edit mode
      if (!this.isPreviewMode) {
    ...
    // Add "Add Column" button as the last header cell (only in edit mode)
    if (!this.isPreviewMode) {
    ...
    this.data.rows.forEach((row, rowIndex) => {
      ...
      row.forEach((cell, colIndex) => {
        ...
        td.textContent = cell
        td.contentEditable = this.isPreviewMode ? 'false' : 'true'
        td.spellcheck = false
      ...
      // Add placeholder cell for the "Add Column" button column (only in edit mode)
      if (!this.isPreviewMode) {
    ...
    // Add "Add Row" button (only in edit mode)
    if (!this.isPreviewMode) {
